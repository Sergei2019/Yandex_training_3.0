...
Ученики знают, что варианты будут раздаваться строго по порядку: правый относительно учителя ученик первой парты получит вариант 1, левый — вариант 2, 
правый ученик второй парты получит вариант 3 (если число вариантов больше двух) и т.д. Так как K может быть меньше чем число учеников N, то после варианта K снова выдаётся вариант 1. 
...
num_pl = 2*row if num==2 else 2*row-1

if (num_pl-K>K/2 and N < 2*K and num_pl != N) or N==K:
    print(-1)
elif row==1 and num==1:
    ...
    if K < num_pl:
        ...
        num_do = num_pl - K
        num_posle = num_pl + K
        row_do = math.ceil(num_do/2)
        row_posle = math.ceil(num_posle/2)
        if num_posle > N:
            num_place = 2 if num_do%2==0 else 1
            print(row_do, num_place)
        else:
            if abs(row_posle-row) <= abs(row_do-row):
                num_place = 2 if num_posle % 2 == 0 else 1
                print(row_posle, num_place)
            else:
                num_place = 2 if num_do % 2 == 0 else 1
                print(row_do, num_place)
    elif num_pl + K <= N:
